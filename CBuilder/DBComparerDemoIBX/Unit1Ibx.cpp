...
void __fastcall TForm1::ExtractMasterDatabase()
{
  ...
  try
  {
    DataMdIbx->DBCConnection1->Connected = true;
  }
  catch (Exception &E)
  {
    Dialogs::ShowMessage(E.Message);
    ...
    ErrFlg = true;
    DataMdIbx->IBDatabase1->Connected = false;
  }
  if (ErrFlg) return;

  MemoExtr->Lines->Add("Database: " + DataMdIbx->IBDatabase1->DatabaseName + " open.");
  DBStructure1->Clear();
  IBDBExtract1->ExtractDatabase();
  DataMdIbx->DBCConnection1->Connected = false;
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::ExtractTargetDatabase()
{
  ...
  try
  {
    DataMdIbx->DBCConnection2->Connected = true;
  }
  catch (Exception &E)
  {
    Dialogs::ShowMessage(E.Message);
    MemoExtr->Lines->Add("Error: " + E.Message);
    ErrFlg = true;
    DataMdIbx->IBDatabase2->Connected = false;
  }
  if (ErrFlg) return;

  MemoExtr->Lines->Add("Database: " + DataMdIbx->IBDatabase2->DatabaseName + " open.");
  DBStructure2->Clear();
  IBDBExtract2->ExtractDatabase();
  DataMdIbx->DBCConnection2->Connected = false;
}
//---------------------------------------------------------------------------
